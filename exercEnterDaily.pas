...
var index: integer;
    isError: boolean;
begin
  isError:= validateDailyInput(3, edt_input.Text); //validate input [index 3]
...
//changing combobox to show track page
procedure Tform_exercEnterDaily.cbo_selectViewChange(Sender: TObject);
begin
if cbo_selectView.itemIndex = 1 then
  begin
  form_exercTrack.show; //show track page
  form_exercEnterDaily.Hide;   //hide current form
  end;
end;
//creating form
...
mmo_comment.ReadOnly; //encouraging comment read only
end;
...
//move mouse off 'back to dashboard'
